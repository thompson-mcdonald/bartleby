...
const { validateRequired, validateNumber, validatePositiveInteger, validateDate } = require('./validators');
...
async function generateInvoice() {
  ...
  const confirmed = await reviewAndConfirm(client, invoiceDetails, items, config, paymentMethod);
  ...
  while (addingItems) {
    ...
    if (items.length > 0) {
      ...
      items.forEach((item, index) => {
        console.log(`  ${index + 1}. ${item.description} - Qty: ${item.quantity} @ $${item.rate.toFixed(2)} = $${item.amount.toFixed(2)}`);
      });
      console.log('');
    }
...
async function reviewAndConfirm(client, invoiceDetails, items, config, paymentMethod) {
  const subtotal = items.reduce((sum, item) => sum + item.amount, 0);
  const total = subtotal;
  ...
  while (reviewing) {
    ...
    items.forEach((item, index) => {
      ...
      console.log(`     Qty: ${item.quantity} @ $${item.rate.toFixed(2)} = $${item.amount.toFixed(2)}`);
    });
